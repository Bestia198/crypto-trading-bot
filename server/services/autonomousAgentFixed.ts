/**
 * Fixed Autonomous Agent Service
 * Enhanced with timeout handling and better error recovery
 */

import { invokeLLM } from "../_core/llm";
import { z } from "zod";

// Zod schema for strategy recommendation validation
...
// Zod schema for trading signal validation
 ...
 * FIXED: Added timeout handling and Zod validation
 */
export async function selectOptimalStrategy(
  ...
  try {
    ...
    const avgPrice =
      marketData.priceHistory.reduce((a, b) => a + b, 0) / marketData.priceHistory.length;
...
Based on the market conditions, recommend the BEST strategy. Respond in JSON format:
{
  ...
  "parameters": {
    ...
    // FIXED: Added Zod validation before using parsed data
    const parsed = JSON.parse(content);
    const validated = StrategyRecommendationSchema.parse(parsed);
